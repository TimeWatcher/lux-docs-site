...
Lux 的编辑器支持由 `lux-vscode` 扩展启动选定 compiler 的 `luxc lsp` 提供。扩展本身保持很薄：负责启动 compiler language server、贡献语法和命令、展示 server 返回结果。Lux module resolution、realm 检查、import/export completion、hover、diagnostics 和 GMod API 智能都来自实际用于 build 的同一个 compiler 版本。
