---
title: Getting Started
description: Install prerequisites, compile a single file, create a GMod project, build artifacts, run diagnostics, and map Lua errors back to Lux.
---

# Getting Started

This guide walks through the smallest useful Lux workflow during the alpha:
build `luxc` from source, compile one file, write a multi-part module, then
build a GMod addon layout.

## Requirements

Lux currently targets Lua 5.1 / GLua. For normal addon work you need:

- A Rust toolchain to build `luxc` while no public binary release is active.
- A Garry's Mod addon directory when testing generated artifacts in game.
- Optional `gmad.exe` when packaging `.gma` files.

Build the compiler:

```powershell
git clone https://github.com/TimeWatcher/lux.git
cd lux\compiler
cargo build --release
.\target\release\luxc.exe --help
```

For shorter commands, add `compiler\target\release` to `PATH`, then use:

```powershell
luxc --help
```

The examples below assume `luxc` is on `PATH`. Replace it with
`.\target\release\luxc.exe` if you run from the compiler directory.

## Compile One File

Start with a single file:

```text
examples/
  hello.lux
```

```powershell
luxc compile examples\hello.lux
```

Single-file compilation is useful for checking syntax, generated Lua shape, and
small language examples. GMod projects normally use `luxc gmod build` so the
compiler can discover modules, split realms, and emit loaders.

The repository's `examples/features.lux` is deliberately a single-file syntax
tour. It does not teach imports, exports, packages, macros, or top-level realm
ownership. Use `examples/gmod_project` for the project/module model.

## Write a First Module

Create a module directory. A multi-file module needs an entry part.

```text
src/
  inventory/
    module.lux
    sv_store.lux
    cl_panel.lux
```

The entry part is where the public module shape and part order are easiest to
find.

<LuxLuaPair title="Entry part" luxLabel="Lux: module.lux" luaLabel="Generated Lua shape">

```lux
part order { "module", "sv_store", "cl_panel" }

fn normalizeItemId(id) tostring(id)

export { normalizeItemId }
```

```lua
local function normalizeItemId(id)
  return tostring(id)
end

__lux_exports.normalizeItemId = normalizeItemId
```

</LuxLuaPair>

Server parts can use the private function from the entry part without importing
it. All parts in the same module share module-private scope.

<LuxLuaPair title="Server part" luxLabel="Lux: sv_store.lux" luaLabel="server artifact">

```lux
server fn grantItem(ply, itemId) {
  local normalized = normalizeItemId(itemId)
  print(`grant ${normalized} to ${ply:Nick()}`)
}

export server { grantItem }
```

```lua
local function grantItem(ply, itemId)
  local normalized = normalizeItemId(itemId)
  return print("grant " .. tostring(normalized) .. " to " .. tostring(ply:Nick()))
end

__lux_exports.grantItem = grantItem
```

</LuxLuaPair>

Client parts can expose client-only UI without leaking it into the server
artifact.

<LuxLuaPair title="Client part" luxLabel="Lux: cl_panel.lux" luaLabel="client artifact">

```lux
client fn openInventoryPanel() {
  local panel = vgui.Create("DFrame")
  panel:SetTitle("Inventory")
  panel:MakePopup()
}

export client { openInventoryPanel }
```

```lua
local function openInventoryPanel()
  local panel = vgui.Create("DFrame")
  panel:SetTitle("Inventory")
  panel:MakePopup()
end

__lux_exports.openInventoryPanel = openInventoryPanel
```

</LuxLuaPair>

The module has one public API, but each exported name is available only in the
realm where it is valid.

## Create a GMod Project

Create a project skeleton. Plain `init` is offline and writes no dependencies:

```powershell
luxc init my_addon
```

Add the official standard package set when your project imports packages such
as `@lux/std` or `@lux/gmod`:

```powershell
luxc init my_addon --std
Push-Location my_addon
luxc install @lux/gmod --from github:TimeWatcher/lux-packages
Pop-Location
```

Create `lux.toml` at the project root:

```toml
package_id = "my_addon"
bundle_id = "my_addon"

[target.gmod]
source_root = "src"
out = "generated/lua"
runtime_base = "lux/my-addon"
autorun = true
source_comments = "readable"

[dependencies]
```

Lux has no package registry. `lux.toml` chooses concrete package sources with
`github`, `url`, or `path`, and can pin GitHub package sets with `tag`,
`branch`, or `commit`. `lux.lock` records the resolved package graph.

Recommended layout:

```text
my_addon/
  lux.toml
  src/
    inventory/
      module.lux
      sv_store.lux
      cl_panel.lux
  generated/
    lua/
      autorun/
      lux/
```

Build GMod artifacts:

```powershell
luxc gmod build --manifest lux.toml
```

The build step discovers modules under `source_root`, infers realms from file
prefixes and folders, resolves package imports, emits Lua artifacts under
`out`, writes a relative-path-safe Lux loader tree under `runtime_base`, and
writes an optional addon `autorun` forwarder.

## Build from Explicit Paths

For experiments and CI slices, you can build from an explicit source root and
output root:

```powershell
luxc gmod build src --out generated/lua
```

Use a manifest for real projects. It keeps package ids, output roots, source map
settings, autorun policy, and extern policy in one stable place.

## Check and Format

Run linting without emitting artifacts:

```powershell
luxc lint src\inventory\module.lux
```

Format a file:

```powershell
luxc format src\inventory\module.lux --write
```

Format whole project sources through your editor or a script until the CLI grows
full project formatting commands.

## Map Runtime Errors

GMod reports Lua stack traces. Lux emits source maps so you can map generated
Lua locations back to Lux source.

```powershell
luxc map-error generated\lua\lux\client\my_addon\inventory.lua.map.json 42
```

With readable source comments enabled, generated Lua also contains source
anchors around functions and branch blocks. Use `source_comments = "none"` for
production-sized artifacts and sidecar source maps for debugging.
