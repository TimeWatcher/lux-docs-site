...
Download the current alpha release from the Lux repository, or build the
...
The self-installed user entrypoint is `%USERPROFILE%\.lux\bin\luxc.exe` on
Windows. If you skip `self install`, call the explicit build output instead:
