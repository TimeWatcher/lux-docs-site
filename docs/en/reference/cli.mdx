...
No public binary release is currently active. During the alpha, build `luxc`
from source and either add `compiler\target\release` to `PATH` or use the
...
If `luxc` is not on `PATH`, use the explicit build output:
...
Once binary releases are active, install or update explicit versions:
